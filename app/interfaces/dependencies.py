...
#     """Provide CreateTask use case with injected repository."""
#     return CreateTask(repo)


# def get_list_tasks_uc(repo: ITaskRepository = Depends(get_task_repo)) -> ListTasks:
#     """Provide ListTasks use case with injected repository."""
...
def get_task_repo(session: AsyncSession = Depends(get_db)) -> ITaskRepository:
    """Provide concrete repository implementation for tasks."""
    # Lazy import to avoid loading SQL stack during test collection
    from app.infrastructure.repositories.sql.task_repository import TaskRepositorySQL  # type: ignore
    return TaskRepositorySQL(session)
